...
typedef struct node_nw_prop_{
    ...
    rt_table_t *rt_table;
    /*L3 properties*/ 
    bool_t is_lb_addr_config; // is loopback address configured
...
static inline void
init_node_nw_prop(node_nw_prop_t *node_nw_prop) { // this function defines the default values for the node_nw_prop
...
typedef struct intf_nw_props_ {
    ...
    /*L3 properties*/
    bool_t is_ipadd_config; // set to TRUE if IP address is configured on this interface
    ip_add_t ip_add;
    char mask;
} intf_nw_props_t;

// this functions defines the default values for the intf_nw_props
static inline void 
...
#define IF_MAC(intf_ptr)   ((intf_ptr)->intf_nw_props.mac_add.mac) // macro for mac address
#define IF_IP(intf_ptr)    ((intf_ptr)->intf_nw_props.ip_add.ip_addr) // macro for ip address

#define NODE_LO_ADDR(node_ptr) (node_ptr->node_nw_prop.lb_addr.ip_addr) // macro for loop back address
...
bool_t node_set_loopback_address(node_t *node, char *ip_addr); // used to configure loopback address
bool_t node_set_intf_ip_address(node_t *node, char *local_if, char *ip_addr, char mask); // used to configure ip address on interface and mask is used to configure subnet mask
bool_t node_unset_intf_ip_address(node_t *node, char *local_if); // used to uncounfigure ip address on interface
